...
struct Point {
    ...
    Point normalized() const {
        ...
        if (mag < EPS) return {0, 0};  // Avoid division by zero
        return {x / mag, y / mag};
    }
...
struct Track {
    ...
    void addLeftPoint(const Point pnt) {
        leftBoundary.push_back(pnt);
    }

    void addRightPoint(const Point pnt) {
        rightBoundary.push_back(pnt);
    }
};
